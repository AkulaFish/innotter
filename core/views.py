...
from innotter.permissions import (IsOwnerAdminModerOrReadOnlyOrBlocked,
                                  PostIsOwnerAdminModerOrReadOnlyOrBlocked)
...
class PageViewSet(
    ...
    def get_queryset(self):
        ...
        if username:
            queryset = queryset.get(owner=User.objects.all()
...
class NewsFeedSet(
    ...
    def get_queryset(self):
        ...
        queryset = [
            page.posts.all() for page
            in self.request.user.follows.all()
        ]
        return queryset
