...
def test_page_update(client, user, user_page, tag):
    ...
    client.login(username="user", password="userpass")
    response = client.put("/api/pages/2/", payload, format="json")
    assert response.data["tags"] == [OrderedDict(
        {
            "id": 1,
            "name": "SomeTag"
        }
...
def test_follow_unfollow_page(client, user_page, admin):
    ...
    response = client.put("/api/pages/2/follow-unfollow/")

    assert response.data["response"] == "Now you follow this page."
    assert len(user_page.followers.all()) == 1

    response = client.get("/api/pages/2/follow-unfollow/")
...
def test_send_request_to_page(client, private_user_page, admin):
    ...
    client.login(username="admin", password="adminpass")
    response = client.put("/api/pages/1/follow-unfollow/")
...
def test_accept_single_request(client, admin, private_user_page):
    ...
    client.login(username="user", password="userpass")
    response = client.put("/api/pages/1/requests/accept/3")
...
@pytest.mark.django_db
def test_accept_all_requests(client, user, moderator, admin, private_user_page, ):
    ...
    client.login(username="user", password="userpass")
    response = client.put("/api/pages/1/requests/accept/")
...
def test_decline_single_requests(client, admin, private_user_page):
    ...
    private_user_page.follow_requests.set([admin])
    client.login(username="user", password="userpass")
    response = client.put("/api/pages/1/requests/decline/3")
...
def test_decline_all_requests(client, admin, moderator, user, private_user_page):
    ...
    private_user_page.follow_requests.set([admin, moderator])
    client.login(username="user", password="userpass")
    response = client.put("/api/pages/1/requests/decline/")
...
def test_get_requests(client, private_user_page, user, admin, moderator):
    ...
    private_user_page.follow_requests.set([admin, moderator])
    client.login(username="user", password="userpass")
    response = client.get("/api/pages/1/requests/")

    assert len(response.data) == 2


@pytest.mark.django_db
def test_accept_request_of_not_your_page(client, private_user_page, user, admin, moderator, user2):
...
@pytest.mark.django_db
def test_decline_request_of_not_your_page(client, private_user_page, user, admin, moderator, user2):
...
@pytest.mark.django_db
def test_get_requests_of_not_your_page(client, private_user_page, user, admin, moderator, user2):
...
@pytest.mark.django_db
def test_block_page_without_permission(client, user_page, user2):
    """Testing blocking without admin or moderator privilege"""
    payload = {
        "permanent_block": True,
        "unblock_date": "2030-11-30T00:00:00+01:00"
    }
    client.login(username="user2", password="userpass")
    response = client.put("/api/block-page/2/", payload, format="json")
    assert response.status_code == 403
